function output = MCGlobalSensitivity(srgtSRGT, Xa, Xb)
...
%for c1 = 1 : nMCpoints
    ya = sim(srgtSRGT,Xa')';
    yb = sim(srgtSRGT,Xb')';
    %eval(sprintf('ya(c1,1) =  sim(model_rbf,Xa(c1,:)')'(Xa(c1,:), srgtSRGT);', SRGT));
    %eval(sprintf('yb(c1,1) = srgts%sEvaluate(Xb(c1,:), srgtSRGT);', SRGT));
    for c2 = 1 : ndv
        %eval(sprintf('yc(c1,1,c2) = srgts%sEvaluate(C(c1,:,c2), srgtSRGT);', SRGT));
        yc(:,1,c2) = sim(srgtSRGT,C(:,:,c2)')';
    end
%end

% compute mean values
fmean0 = (sum(ya(:,1))/nMCpoints)^2;

% calculate individual sensitivity indices
Sindividual = zeros(ndv,1);

fmean1=mean([mean(ya),mean(yb),mean(mean(yc))])^2;
fmean2=min(fmean1,fmean0);
D=(dot(ya(:,1),ya(:,1))/nMCpoints - fmean2);
for c1 = 1 : ndv
    % first order sensitivity (individual effect)
    Sindividual(c1) = (dot(ya(:,1),yc(:,1,c1))/nMCpoints - fmean2)/D;
end
Sindividual(Sindividual<0)=1e-4;
